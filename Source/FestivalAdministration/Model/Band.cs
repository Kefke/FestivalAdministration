    ...
    class Band
    {
        ...
        public static void AddBand(string name, string twitter, string facebook, string picture, string description)
        {
            ...
            try
            {
                ...
                // Get ID from db
                int id = 0;
                DbDataReader reader = Database.GetData("SELECT ID FROM band WHERE Name = @name AND Twitter = @twitter AND Facebook = @facebook AND Picture = @picture AND Description = @description", param1, param2, param3, param4, param5);
                foreach (DbDataRecord record in reader)
                {
                    // Get ID
